...
const FeaturedCard = memo(function FeaturedCard({ project }: { project: Project }) {
  ...
  return (
    <motion.div variants={fadeUp} className="card group flex flex-col" data-project-id={project.id}>
      <ProjectMedia
        image={project.image}
        video={project.video}
        title={project.title}
        ...
        <div className="flex gap-2.5 mt-auto">
          {project.video ? (
            <button
              type="button"
              onClick={() => {
                // Find and click the play button inside the card
                const card = document.querySelector(`[data-project-id="${project.id}"] button[aria-label*="Play"]`) as HTMLButtonElement | null;
                card?.click();
                // Scroll to card top so user sees video
                document.querySelector(`[data-project-id="${project.id}"]`)?.scrollIntoView({ behavior: "smooth", block: "nearest" });
              }}
